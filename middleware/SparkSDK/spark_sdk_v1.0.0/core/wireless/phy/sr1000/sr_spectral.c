...
void config_spectrum(calib_vars_t *calib_vars, uint16_t frequency, tx_power_t tx_power, rf_channel_t *rf_channel)
{
    ...
    set_pulses(calib_vars, rf_channel->channel.tx_pattern, &rf_channel->pulse_size, start_pos, pulse_count, pulse_spacing, (uint8_t)pulse_width, frequency, true);
...
void config_spectrum_advance(calib_vars_t *calib_vars, channel_cfg_t *channel_cfg, rf_channel_t *rf_channel)
{
    ...
    set_pulses(calib_vars,
               rf_channel->channel.tx_pattern,
               &rf_channel->pulse_size,
               channel_cfg->pulse_start_pos,
               channel_cfg->power->pulse_count,
               channel_cfg->pulse_spacing,
               (uint8_t)pulse_width,
               channel_cfg->frequency,
