...
uint32_t wps_stats_get_phy_cca_fail_count(wps_connection_t *connection)
{
    wps_stats_t temp;

    save_object(&connection->wps_stats, &temp, sizeof(wps_stats_t));

    return temp.cca_fail;
}
...
float wps_stats_get_phy_cca_pass_ratio(wps_connection_t *connection)
{
    ...
    return (float)(temp.cca_pass) / (temp.cca_pass + temp.cca_fail);
}
...
float wps_stats_get_phy_cca_fail_ratio(wps_connection_t *connection)
{
    ...
    return (float)(temp.cca_fail) / (temp.cca_pass + temp.cca_fail);
}
