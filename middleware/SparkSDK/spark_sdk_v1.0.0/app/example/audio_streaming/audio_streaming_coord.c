...
#define SAC_FORWARD_CHANNEL_PAYLOAD_SIZE 84
...
static sac_pipeline_t *primary_audio_pipeline;
static sac_pipeline_t *secondary_audio_pipeline;
...
static void app_swc_core_init(swc_error_t *err)
{
    ...
    swc_connection_cfg_t tx_conn_cfg = {
        ...
        .max_payload_size = SAC_FORWARD_CHANNEL_PAYLOAD_SIZE + sizeof(sac_header_t),
...
static void app_audio_core_init(sac_error_t *audio_err)
{
     ...
     * ======================
     *
     * Input:      Stereo stream of 40 samples @ 48 kHz/16 bits is recorded using the MAX98091 hardware audio codec.
     * Processing: None.
     * Output:     Stereo stream of 40 samples @ 48 kHz/16 bits is sent over the air to the Node.
     *
     * +-------+    +-----+
     * | Codec | -> | SWC |
     * +-------+    +-----+
    ...
    audio_endpoint_cfg_t max98091_producer_cfg = {
        ...
        .audio_payload_size = SAC_FORWARD_CHANNEL_PAYLOAD_SIZE,
    ...
    primary_audio_pipeline = sac_pipeline_init("Codec -> SWC", max98091_producer,
                                               primary_pipeline_cfg, swc_consumer, audio_err);
    if (*audio_err != SAC_ERR_NONE)
    {
        return;
    }
    ...
    upsampling_instance.cfg.ratio = AUDIO_SRC_THREE;
    upsampling_instance.cfg.is_divider = false;
    ...
    upsampling_processing = sac_processing_stage_init((void *)&upsampling_instance, "Audio Upsampling", upsampling_iface, audio_err);
    if (*audio_err != SAC_ERR_NONE)
    {
        return;
    }
    ...
    sac_pipeline_cfg_t secondary_pipeline_cfg = {
        ...
        .user_data_enable = false};
    secondary_audio_pipeline = sac_pipeline_init("SWC -> Codec", swc_producer,
                                                 secondary_pipeline_cfg, max98091_consumer, audio_err);
    if (*audio_err != SAC_ERR_NONE)
    {
        return;
    }

    sac_pipeline_add_processing(secondary_audio_pipeline, upsampling_processing);
    sac_pipeline_add_processing(secondary_audio_pipeline, volume_processing);
