...
static void app_audio_core_init(sac_error_t *audio_err)
{
     ...
     * ======================
     *
     * Input:      Stereo stream of 40 samples @ 48 kHz/16 bits is received over the air from the Coordinator.
     * Processing: Digital volume control followed by clock drift compensation.
     * Output:     Stereo stream of 40 samples @ 48 kHz/16 bits is played using the MAX98091 hardware audio codec.
     *
     * +-----+    +----------------+    +-----+    +-------+
    ...
    primary_audio_pipeline = sac_pipeline_init("SWC -> Codec", swc_producer,
                             primary_pipeline_cfg, max98091_consumer, audio_err);
    if (*audio_err != SAC_ERR_NONE) {
        return;
    }
    ...
    downsampling_instance.cfg.is_divider = true;
    ...
    secondary_audio_pipeline = sac_pipeline_init("Codec -> SWC", max98091_producer,
                                                 secondary_pipeline_cfg, swc_consumer, audio_err);
    if (*audio_err != SAC_ERR_NONE) {
        return;
    }
