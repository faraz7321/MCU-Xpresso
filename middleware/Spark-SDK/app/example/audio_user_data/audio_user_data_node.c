...
static audio_user_data_instance_t user_data_rx_instance;
...
static void user_data_rx_callback(uint8_t data);
...
static void app_audio_core_init(sac_error_t *audio_err)
{
    ...
    user_data_rx_instance.mode = AUDIO_USER_DATA_RX,
    user_data_rx_instance.rx_callback = user_data_rx_callback;
    user_data_rx_processing = sac_processing_stage_init((void *)&user_data_rx_instance, "User Data RX", user_data_rx_iface, audio_err);
    if (*audio_err != SAC_ERR_NONE) {
        return;
    }
    ...
    sac_pipeline_add_processing(audio_pipeline, user_data_rx_processing);
...
/** @brief Callback when user data is received.
 */
static void user_data_rx_callback(uint8_t data)
{
    if (data) {
