...
static void app_swc_core_init(swc_error_t *err)
{
    ...
    /* ** RX Coordinator Beacon ** */
    swc_connection_cfg_t rx_beacon_conn_cfg = {
        .name = "RX Beacon Connection",
        ...
        .timeslot_id = rx_beacon_timeslots,
        .timeslot_count = ARRAY_SIZE(rx_beacon_timeslots),
    ...
    rx_beacon_conn = swc_connection_init(node, rx_beacon_conn_cfg, &hal, err);
    if (*err != SWC_ERR_NONE) {
        return;
    }
    ...
    swc_connection_set_tx_success_callback(tx_to_coord_conn, conn_tx_success_callback);
...
static void app_audio_core_init(sac_error_t *audio_err)
{
    ...
    audio_pipeline = sac_pipeline_init("Codec -> SWC", max98091_producer,
                                       audio_pipeline_cfg, swc_consumer, audio_err);
    if (*audio_err != SAC_ERR_NONE) {
        return;
    }
