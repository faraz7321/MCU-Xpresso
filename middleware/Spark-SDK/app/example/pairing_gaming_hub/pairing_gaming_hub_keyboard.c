...
static bool keyboard_press_toggle;
...
static void send_keyboard_press_button(void);
...
int main(void)
{
    ...
    while (1) {
        ...
        if (!is_device_paired) {
            ...
            iface_button_handling(send_keyboard_press_button, enter_pairing_mode);
        } else {
            iface_button_handling(send_keyboard_press_button, unpair_device);
        }
    }
}
...
/** @brief Toggle the value of a bool and send it over SWC to simulate a keyboard press.
 */
static void send_keyboard_press_button(void)
{
    swc_error_t swc_err;
    keyboard_press_toggle = !keyboard_press_toggle;

    /* Send the payload through the wireless core */
    swc_connection_send(tx_to_dongle_conn, (uint8_t *)&keyboard_press_toggle, sizeof(keyboard_press_toggle), &swc_err);
}
