...
static void app_swc_core_init(swc_error_t *err)
{
    ...
    rx_from_coord_conn = swc_connection_init(node, rx_conn_cfg, &hal, err);
    if (*err != SWC_ERR_NONE) {
        return;
    }
    ...
    swc_connection_set_rx_success_callback(rx_from_coord_conn, conn_rx_success_callback);
