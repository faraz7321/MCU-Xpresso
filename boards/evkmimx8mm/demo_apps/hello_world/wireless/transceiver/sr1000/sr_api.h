 ...
 *  @param[in]  radio_hal            Radio HAL instance.
              ...
              sr_api_error_t *error)
{
    *error = API_ERR_NONE;
    ...
    peripherals_ctrl_t periph_ctrl = radio->std_spi;

    uwb_control_peripherals(radio, SET_CTRL_PERIPH(CTRL_PERIPH_CLEAR, periph_ctrl));
...
static inline void uwb_set_syncword_config(radio_t *radio, syncword_cfg_t *cfg)
{
    ...
    reg_data.short_val = cfg->syncword_length                              |
...
static inline void uwb_select_channel(radio_t *radio, uint8_t *tx_pattern, uint8_t pulse_size)
{
    ...
    sr_access_append_write_burst(&radio->access_sequence,
                                 REG_TXPULSE12 + write_pulse_size,
                                 tx_pattern + write_pulse_size,
...
{
    sr_access_spi_transfer_non_blocking(radio->radio_hal,
                                        tx_buffer,
                                        radio->access_sequence.rx_buffer,
                                        size);
}
...
{
    sr_access_spi_transfer_blocking(radio->radio_hal,
                                    tx_buffer,
                                    radio->access_sequence.rx_buffer,
                                    size);
}
...
static inline void uwb_set_timer_cfg_burst(radio_t *radio, uint16_t sleep_time, uint16_t rx_timeout, uint8_t rx_pwr_up_delay, int16_t pwr_up)
{
    sr_phy_error_t access_sequence_error = ACCESS_SEQUENCE_ERR_BUSY;
